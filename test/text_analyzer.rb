...
def list_companies(raw_company_list)
  ...
  companies.each_with_index do |company, _index|
...
def find_company_reports(report_type, company_cik); end
...
def read_report
  ...
  doc_html.search('span').each do |paragraph|
    puts paragraph.text if paragraph.text.include?('climate change')
  end
end
